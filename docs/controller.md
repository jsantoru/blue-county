...
`npm run test:controller-drift` tests actual gameplay both at startup and after deliberate stick use. Small diagonal resting offsets produce zero walking/look input and no camera rotation for 120 frames. Gentle stick movement in all four directions turns the character toward travel; the previous extra facing threshold allowed movement without turning. See [the drift and gentle-walking report](controller-drift-verification.json). Older saves inherit the new radial defaults or their stronger previous shared deadzone; explicit walking/camera settings persist independently.

The controller fix passes 44 focused input tests and the full 165-test suite. `npm run test:controller-transitions` exercises the actual production render loop with a synthetic controller whose right stick remains at 0.30 throughout, above the camera deadzone. Three exits begin walking with left stick and Y still held; three entries begin driving with RT already held. Startup detection, first fresh A, deliberate resume after focus loss, automatic reconnect and held-Y debouncing all pass with zero browser/WebGL errors. See [the regression report](controller-transition-verification.json). These checks verify software behavior; the user's physical controller still needs a playtest.
