...
- Three user-supplied street photographs now support Home's front facade and yard. The owner separately confirms a deck on the viewer-right/driveway side, a patio and red door below, sliders directly above, and a wraparound rear deck with a roofed screened area. These refine the rendering while retaining Home's source footprint and the previously traced driveway. See the [Home detail pass](home-detail-pass.md) and [photo observations](research-streetview/home-photo-observations.md).
...
Geometry uses **one meter per game unit**, local **X east, Y up, Z south**. Latitude/longitude are projected with a local equirectangular approximation about the house point. Heading is `atan2(dx, dz)` in radians; zero faces south (+Z). Stored Home parking-apron position is `[0.207, -1.165, 19.601]`; its initial heading is `-0.240534` radians. The runtime terrain sampler supplies the final contact height.
...
Within the **428 × 630m Beverly reference area**, 64 NYS footprints replace the earlier approximate address-point buildings, including garages and sheds. Most footprint source dates are 2013, compared with spring 2025 reference imagery; the newer county dataset publication date does not make every outline current. Source polygons are retained, while rendered envelopes, roof forms and heights remain approximate. Public parcel geometry supports address/style association, not ownership or a surveyed setback. Inspected exterior listing photographs support limited facade observations for eight houses: numbers 12, 20, 22, 26, 29, 34, 35 and 41. The three supplied Home photographs add number 2, bringing the count to **nine photo-referenced facades**. Other facade colors and unseen details remain provisional. Full Street View coverage was not obtained.

The compiled neighborhood has **37 driveway approaches** and **122 observed crown positions**. Occluded segments retain confidence notes. Home's full curved approach is reconstructed from official 2010/2013 aerials and its exposed 2025 parking apron; its earlier mouth-only stub is superseded. The original canopy observations remain crowns, not measured trunks: obstructed trunk candidates are omitted rather than moved onto a road or building. Home's front yard now uses three mature deciduous trees with high, mostly bare crowns, replacing conflicting generic foreground trees. Their coordinates and heights are photo-perspective estimates, not measured tree inventory. Woods and ground-cover shapes guide procedural scenery; aerial photographs are not game textures. Outside the reference area, OSM footprints and flagged address-point envelopes retain the broader generic interpretation. Mode-specific race arrows and barriers are game objects.

At Home, the low front planting beds, narrow walk toward the driveway, bench, decorative wheel, pale pedestal/birdbath and numbered mailbox follow the supplied views. The confirmed deck/patio arrangement has inferred dimensions and construction details; the hidden screen-room roof and panel arrangement are representative. Its custom deck replaces the generic rendering of one existing deck observation, leaving **11 generic decks plus the custom Home deck**. Home's lower patio is additional to the three prior observed patios. The coarse DEM cannot recover actual small garden grades: terrain-following mulch and walk geometry use the same terrain facets, while level patio/deck construction has a supporting skirt and posts. These local details do not alter source building polygons or driveway pavement.
...
**Beverly references: NYS ITS Geospatial Services / NYSDOP, dated 2010–2025; building footprints: NYS ITS Geospatial Services, Orange County GIS Division, NYSERDA and contributing sources; parcel reference: Orange County and NYS ITS Geospatial Services.** The state offers no-cost public GIS services and exports with as-is/no-warranty metadata; no named CC0 or blanket public-domain license was established for these resources. Source-specific terms and dates remain documented in [Beverly reconstruction](beverly-reconstruction.md#sources-and-resource-terms) and the cached item metadata. Original aerial imagery remains a research reference; ordinary listing photographs are linked and described, not bundled. Neither is a game texture. The three user-supplied Home street screenshots are described as observations only and are not copied into the repository or used as textures. No Google imagery is included in the game.
