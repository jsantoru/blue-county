...
| Ed | Blue 1953 Buick woody wagon | Owner's description. Blue body, original generated wood grain and framing, tall wagon cabin, whitewalls, tooth grille and fender portholes. |

The new four cars are original stylized approximations, not scans or concours-accurate models. Their body sections, period trim and cabin details need further hand modeling for a hero-quality close-up. Shared arcade seat proportions keep the driver aligned with the wheel. Only Joe has a referenced human likeness; the friends use distinct neutral stand-ins, with colors/hair chosen for visual identification rather than as claims about their appearance. No supplied photo pixels or real registration plates are distributed as game textures.
...
- `scripts/build-club-cars.py`: original geometry, materials, GLB export, manifests and matching studio renders. `npm run export-club-cars` runs Blender in the background. Regeneration overwrites the generated source, so preserve any manual Blender edits first.
...
The final unit suite passes **206 tests in 26 files**, including 17 actual-GLB checks that cast rays through opened doorways, measure the exported tire geometry and test the driver's head against the real roof. See [the saved browser report](lug-nuts-verification.json) and [the project verification history](verification.md).
