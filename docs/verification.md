...
The current Beverly reconstruction builds successfully with TypeScript, Vite 8.3, Three.js 0.186 and Rapier 0.20. Dependency versions are locked in `package-lock.json`. **50 tests across seven files pass**, and the full browser race/restart/Home flow was repeated successfully in Edge 153.0.4234.48 with zero page errors and a **6:35.58** race result. The earlier build was also checked in Chrome 152.0.7977.83; its separate report remains `browser-verification-chrome.json`.
...
The subsequent neighborhood visual upgrade also passes all 42 tests, the production build, all three graphics presets and the full browser race/restart/Home flow. Its final race time was 6:35.62. The new 1080p High moving sample measured a 16.70 ms median and 16.80 ms 95th-percentile frame interval on the GTX 1660 SUPER. See `visual-upgrade.md` for the measured scope and `visual-verification.json` for the full report.
