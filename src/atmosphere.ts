...
export class Atmosphere {
  readonly sunDirection = new T.Vector3(-0.75, 0.72, -0.55).normalize();
  private sky = new Sky();
  ...
  constructor(scene: T.Scene, renderer: T.WebGLRenderer) {
    ...
    const cloudMaterial = new T.ShaderMaterial({
      ...
      fragmentShader: `varying vec3 vDirection; uniform float time; uniform vec3 sunDirection;
        ...
        float fbm(vec2 p){float v=0.,a=.52;for(int i=0;i<5;i++){v+=a*noise(p);p=mat2(1.6,-1.2,1.2,1.6)*p;a*=.5;}return v;}
        void main(){vec3 d=normalize(vDirection);if(d.y<.02)discard;
          vec2 p=d.xz/(d.y+.15)*2.1+vec2(time*.003,0.);
          float n=fbm(p), wisps=fbm(p*2.7+4.);
          float alpha=smoothstep(.48,.68,n*.84+wisps*.16)*smoothstep(.02,.20,d.y)*.9;
          vec3 color=mix(vec3(.65,.72,.76),vec3(1.38,1.32,1.2),smoothstep(.48,.67,n));
          color+=vec3(.27,.23,.16)*pow(max(dot(d,sunDirection),0.),8.);
